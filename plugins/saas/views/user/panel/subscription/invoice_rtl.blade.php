...
<body>
    <table class="table table-borderless invoice-top">
        <tbody>
            ...
            <tr>
                <td class="text-left">
                    <p class="invoice-p">
                    {{ $payment_history->pid }}:{{ translate('Payment ID', getLocale()) }}
                    </p>
                    <p class="invoice-p">
                    {{ $payment_history->updated_at }}:{{ translate('Payment date', getLocale()) }}
                    </p>
                    <p class="invoice-p">
                    {{ $payment_history->method }}:{{ translate('Payment method', getLocale()) }}
                    </p>
                </td>
                <td></td>
            </tr>
    ...
    <table class="table invoice-product-table p5">
        <thead class="thead-light">
            <tr>
                <td scope="col" class="text-left invoice-p">{{ translate('Total', getLocale()) }}</td>
                <td></td>
                <td></td>
                <td></td>
                <td scope="col" class="invoice-p text-right">{{ translate('Title', getLocale()) }}</td>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td class="text-left invoice-p currency">
                    {{ currencyExchange($payment_history->primary_amount, true, null, false) }}
                </td>
                <td></td>
                <td></td>
                <td></td>
                <td class="invoice-p text-right">
            ...
            <tr>
                <td colspan="2">
                    <table class="table table-borderless w-100 invoice-summary-table">
                        <tr>
                            <td class="border-top-0 invoice-p p-0 currency text-left">
                                {{ currencyExchange($payment_history->primary_amount, true, null, false) }}
                            </td>
