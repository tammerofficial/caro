...
class RazorpayController extends Controller
{
    ...
    {
        $this->currency = \Plugin\Saas\Repositories\PaymentMethodRepository::configKeyValue(config('saas.payment_methods.razorpay'), 'razorpay_currency');
        $this->total_payable_amount = (new PaymentController())->convertCurrency($this->currency, session()->get('payable_amount'));
        $this->key_id = \Plugin\Saas\Repositories\PaymentMethodRepository::configKeyValue(config('saas.payment_methods.razorpay'), 'razorpay_key_id');
        $this->key_secret = \Plugin\Saas\Repositories\PaymentMethodRepository::configKeyValue(config('saas.payment_methods.razorpay'), 'razorpay_key_secret');
    }
    ...
    public function index()
    {
        ...
        return view('plugin/saas::payments.gateways.razorpay.index', [
            'key_id' => $this->key_id,
            'order' => [
                'id' => $orderId,
                'amount' => $orderAmount,
