...
class SSLCommerzController extends Controller
{
    ...
    {
        $this->currency = \Plugin\Saas\Repositories\PaymentMethodRepository::configKeyValue(config('saas.payment_methods.sslcommerz'), 'sslcz_currency');
        $this->total_payable_amount = (new PaymentController())->convertCurrency($this->currency, session()->get('payable_amount'));
        $this->sslcz_store_id = \Plugin\Saas\Repositories\PaymentMethodRepository::configKeyValue(config('saas.payment_methods.sslcommerz'), 'sslcz_store_id');
        $this->sslcz_store_password = \Plugin\Saas\Repositories\PaymentMethodRepository::configKeyValue(config('saas.payment_methods.sslcommerz'), 'sslcz_store_password');
        $this->sandbox = \Plugin\Saas\Repositories\PaymentMethodRepository::configKeyValue(config('saas.payment_methods.sslcommerz'), 'sandbox');
    ...
    public function index()
    {
        ...
        $post_data['value_c'] = DB::table('sessions')->insertGetId(['payload' => json_encode(session()->all(), true)]);

        $post_data['success_url'] = route('plugin.saas.sslcommerz.success.payment');
        $post_data['fail_url'] = route('plugin.saas.sslcommerz.fail.payment');
        $post_data['cancel_url'] = route('plugin.saas.sslcommerz.cancel.payment');
    ...
    {
        $session_data = DB::table('sessions')->where('id', '=', $request->value_c)->value('payload');

        $sessionData = json_decode($session_data);
