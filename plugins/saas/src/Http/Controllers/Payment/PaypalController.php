...
class PaypalController extends Controller
{
    ...
    {
        $this->currency = \Plugin\Saas\Repositories\PaymentMethodRepository::configKeyValue(config('saas.payment_methods.paypal'), 'paypal_currency');
        $this->total_payable_amount = (new PaymentController())->convertCurrency($this->currency, session()->get('payable_amount'));
        $this->paypal_client_id = \Plugin\Saas\Repositories\PaymentMethodRepository::configKeyValue(config('saas.payment_methods.paypal'), 'paypal_client_id');
        $this->paypal_client_secret = \Plugin\Saas\Repositories\PaymentMethodRepository::configKeyValue(config('saas.payment_methods.paypal'), 'paypal_client_secret');
        $this->is_active_sandbox = \Plugin\Saas\Repositories\PaymentMethodRepository::configKeyValue(config('saas.payment_methods.paypal'), 'sandbox');
    }

    /**
     * Initialize payment for paypal
     */
    public function index()
    {
        ...
        $request->body = [
            ...
            "application_context" => [
                "cancel_url" => route('plugin.saas.paypal.cancel'),
                "return_url" => route('plugin.saas.paypal.success')
            ]
        ];
        try {
