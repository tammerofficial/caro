...
class GpayController extends Controller
{
    ...
    {
        $this->currency = \Plugin\Saas\Repositories\PaymentMethodRepository::configKeyValue(config('saas.payment_methods.gpay'), 'gpay_currency');
        $this->total_payable_amount = (new PaymentController())->convertCurrency($this->currency, session()->get('payable_amount'));

        $this->marchant_id = \Plugin\Saas\Repositories\PaymentMethodRepository::configKeyValue(config('saas.payment_methods.gpay'), 'gpay_marchant_id');
        $this->marchant_name = \Plugin\Saas\Repositories\PaymentMethodRepository::configKeyValue(config('saas.payment_methods.gpay'), 'gpay_marchant_name');
        $sandbox = \Plugin\Saas\Repositories\PaymentMethodRepository::configKeyValue(config('saas.payment_methods.gpay'), 'sandbox');
    ...
    public function index()
    {
        ...
        return view('plugin/saas::payments.gateways.gpay.index', $data);
    }
