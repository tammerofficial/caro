...
class UpdateController extends Controller
{
    ...
    public function updateSystem(Request $request)
    {
        ...
        $sourcePath = base_path('updates');

        if (File::exists($sourcePath)) {
            if (!File::exists($temp_storage_path)) {
                File::makeDirectory($temp_storage_path.'/updates', 0755, true, true);
            }
            File::move($sourcePath, $temp_storage_path.'/updates');
        }

        $redirect_route = 'admin.dashboard';
        return $this->processUpdate($temp_storage_path, $temp_storage_path, $redirect_route);
    }
    ...
    public function processUpdate($root_path, $rm_dir, $redirect_route = null)
    {
        try {
            ...
            if (isset($json['sql']) && !empty($json['sql'])) {
                $tenants = DB::table('tl_saas_packages')
                    ->join('tl_saas_accounts', 'tl_saas_accounts.package_id', '=', 'tl_saas_packages.id')
                    ->where('is_db_created', '=', 1);
                $tenants->update([
                    'tl_saas_accounts.is_system_db_updated' => 0,
                    'tl_saas_accounts.status' => 0
                ]);

                $all_tenants = $tenants->pluck('tenant_id');
                ...
                foreach ($sql_files as $sql) {
                    ...
                    if (file_exists($sql_path)) {
                        ...
                        if ($sql['type'] == 'saas' || $sql['type'] == 'both') {
                ...
                for ($i = 0; $i < sizeof($all_tenants); $i++) {
                    $job = new UpdateTenantDatabaseJob($all_tenants[$i], $sql_files, false);
                    dispatch($job);
                }
            }
