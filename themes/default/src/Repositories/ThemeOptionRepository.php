...
class ThemeOptionRepository
{
    ...
    private function setHeaderOptionsStyle($style)
    {
        ...
        foreach ($style as $key => $value) {
            ...
            if ($key == 'header_text_color' && $value != "") {
                $themeOption['.header .header-fixed h2']['color'] = $value . "!important;";
            }
            if ($key == 'header_sticky_text_color' && $value != "") {
                $themeOption['.header .header-fixed.is-sticky h2']['color'] = $value . "!important;";
            }
            ...
            if ($key == 'header_reg_bg_color' && $value != "") {
                $themeOption['.btn-crs.plug.reg']['background-color'] = $value . "!important;";
            }
            if ($key == 'header_reg_color' && $value != "") {
                $themeOption['.btn-crs.plug.reg']['color'] = $value . "!important;";
                $themeOption['.btn-crs.plug.reg:hover']['color'] = $value . "!important;";
            }

            if ($key == 'sticky_header_reg_bg_color' && $value != "") {
                $themeOption['.is-sticky .btn-crs.plug.reg']['background-color'] = $value . "!important;";
                $themeOption['.mobile .btn-crs.plug.reg']['background-color'] = $value . "!important;";
            }
            if ($key == 'sticky_header_reg_color' && $value != "") {
                $themeOption['.is-sticky .btn-crs.plug.reg']['color'] = $value . "!important;";
                $themeOption['.is-sticky .btn-crs.plug.reg:hover']['color'] = $value . "!important;";

                $themeOption['.mobile .btn-crs.plug.reg']['color'] = $value . "!important;";
                $themeOption['.mobile .btn-crs.plug.reg:hover']['color'] = $value . "!important;";
            }

            if ($key == 'header_login_bg_color' && $value != "") {
                $themeOption['.btn-crs.plug.login']['background-color'] = $value . "!important;";
            }
            if ($key == 'header_login_color' && $value != "") {
                $themeOption['.btn-crs.plug.login']['color'] = $value . "!important;";
                $themeOption['.btn-crs.plug.login:hover']['color'] = $value . "!important;";
            }

            if ($key == 'sticky_header_login_bg_color' && $value != "") {
                $themeOption['.is-sticky .btn-crs.plug.login']['background-color'] = $value . "!important;";
                $themeOption['.mobile .btn-crs.plug.login']['background-color'] = $value . "!important;";
            }
            if ($key == 'sticky_header_login_color' && $value != "") {
                $themeOption['.is-sticky .btn-crs.plug.login']['color'] = $value . "!important;";
                $themeOption['.is-sticky .btn-crs.plug.login:hover']['color'] = $value . "!important;";

                $themeOption['.mobile .btn-crs.plug.login']['color'] = $value . "!important;";
                $themeOption['.mobile .btn-crs.plug.login:hover']['color'] = $value . "!important;";
            }

            if ($key == 'header_dash_bg_color' && $value != "") {
                $themeOption['.btn-crs.plug.dash']['background-color'] = $value . "!important;";
            }
            if ($key == 'header_dash_color' && $value != "") {
                $themeOption['.btn-crs.plug.dash']['color'] = $value . "!important;";
                $themeOption['.btn-crs.plug.dash:hover']['color'] = $value . "!important;";
            }

            if ($key == 'sticky_header_dash_bg_color' && $value != "") {
                $themeOption['.is-sticky .btn-crs.plug.dash']['background-color'] = $value . "!important;";
                $themeOption['.mobile .btn-crs.plug.dash']['background-color'] = $value . "!important;";
            }
            if ($key == 'sticky_header_dash_color' && $value != "") {
                $themeOption['.is-sticky .btn-crs.plug.dash']['color'] = $value . "!important;";
                $themeOption['.is-sticky .btn-crs.plug.dash:hover']['color'] = $value . "!important;";

                $themeOption['.mobile .btn-crs.plug.dash']['color'] = $value . "!important;";
                $themeOption['.mobile .btn-crs.plug.dash:hover']['color'] = $value . "!important;";
            }
        }
        $styleProperties = makeCssProperties($themeOption);
        ...
        if (isset($style['custom_mobile_menu_style']) && $style['custom_mobile_menu_style'] == 1) {
            foreach ($style as $key => $value) {
                if ($key == 'mobile_menu_color' && $value != "") {
                    $themeOption['@media (max-width:575px)']['.mobile .nav li a']['color'] = $value . "!important;";
                }
                if ($key == 'mobile_menu_hover_color' && $value != "") {
                    $themeOption['@media (max-width:575px)']['.mobile .nav li a:hover']['color'] = $value . "!important;";
                }
                if ($key == 'mobile_menu_active_item_color' && $value != "") {
                    $themeOption['@media (max-width:575px)']['.mobile .nav li a.active-menu-item']['color'] = $value . "!important;";
                }
                if ($key == 'mobile_sub_menu_color' && $value != "") {
                    $themeOption['@media (max-width:575px)']['.mobile .nav li .sub-menu li a ']['color'] = $value . "!important;";
                }
                if ($key == 'mobile_sub_menu_hover_color' && $value != "") {
                    $themeOption['@media (max-width:575px)']['.mobile .nav li .sub-menu li a:hover ']['color'] = $value . "!important;";
                }
                if ($key == 'mobile_sub_menu_active_item_color' && $value != "") {
                    $themeOption['@media (max-width:575px)']['.mobile .nav li .sub-menu li a .active-sub-menu-item']['color'] = $value . "!important;";
                }
            }
            $styleProperties = makeCssProperties($themeOption);
        }
            ...
            foreach ($style['css'] as $key => $values) {
                foreach ($values as $property => $value) {
                    if (in_array($key, $page)) {
                        ...
                        $themeOption['.page-title .nav li, .page-title .nav li a'][$property] = $value . '!important;';
                    } elseif ($style['condition']['breadcrumb_hide_show_c'] == '1' && in_array($key, $breadcrumb_activer)) {
                        $themeOption['.page-title .nav li.active'][$property] = $value . '!important;';
                    } elseif ($style['condition']['breadcrumb_hide_show_c'] == '1' && in_array($key, $breadcrumb_divider)) {
                        $themeOption['.page-title .nav li:after'][$property] = $value . '!important;';
                    } else {
                        $themeOption[$key][$property] = $value . ';';
                    }
                }
    ...
    public function setFooterStyle($style)
    {
        ...
        if (isset($style['custom_footer_style']) && $style['custom_footer_style'] == 1) {
            foreach ($style as $key => $value) {
                ...
                if ($key == 'footer_color' && $value != "") {
                    $themeOption['footer']['color'] = $value . "!important;";
                    $themeOption['footer .widget p']['color'] = $value . "!important;";
                    $themeOption['footer .widget h3']['color'] = $value . "!important;";
                }
                if ($key == 'footer_link_color' && $value != "") {
                    $themeOption['.footer a']['color'] = $value . "!important;";
                }
                if ($key == 'footer_link_hover_color' && $value != "") {
                    $themeOption['.footer a:hover']['color'] = $value . "!important;";
                }

                if (isset($style['footer_social_enable']) && $style['footer_social_enable'] == 1) {
                    if ($key == 'footer_social_color' && $value != "") {
                        $themeOption['footer .social-icon a i']['color'] = $value . "!important;";
                    }
                    if ($key == 'footer_social_hover_color' && $value != "") {
                        $themeOption['footer .social-icon a i:hover']['color'] = $value . "!important;";
                    }
                    if ($key == 'footer_social_alignment' && $value != "") {
                        $themeOption['footer .social-icon']['justify-content'] = $value . "!important;";
                    }
                }
                if (isset($style['footer_text_enable']) && $style['footer_text_enable'] == 1) {
                    if ($key == 'copyright_bg_color' && $value != "") {
                        $themeOption['.footer-container .copyright']['background-color'] = $value . "!important;";
                    }
                }
            }
            $styleProperties = makeCssProperties($themeOption);
