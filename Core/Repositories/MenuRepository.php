        ...
        for ($i = 0; $i < sizeof($data); $i++) {
            ...
            if ($data[$i]->icon == null || $data[$i]->icon == 'null' || $data[$i]->icon == '' ||  $data[$i]->icon == ' ') {
                ...
                if (Session::get('api_locale')!=null && Session::get('api_locale') != getGeneralSetting('default_language')) {
                    ...
                    $translated_menu_group_name = getTranslatedMenuTitle($data[$i]->group_id, Session::get('api_locale'), $data[$i]->group_name,true);
                    $data[$i]->group_name = $translated_menu_group_name;
                }
                else{
