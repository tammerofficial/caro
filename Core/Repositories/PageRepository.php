...
class PageRepository
{
    ...
    public function pageCreateUpdate($request, $id = null, $action = null)
    {
        ...
        $page->page_password = !isTenant() ? Crypt::encrypt($request['page_password']) : null;
    ...
    public function deletePage($permalink)
    {
        ...
        if (isActivePluging('pagebuilder') && $page->page_type == 'builder') {
            \Plugin\PageBuilder\Helpers\BuilderHelper::deleteBuilderCssOnPageDelete($permalink, $page->id);
        }
    ...
    public function bulkDeletePage($data)
    {
        ...
        foreach ($pages as $page) {
            $page->delete();
            if (isActivePluging('pagebuilder') && $page->page_type == 'builder') {
                \Plugin\PageBuilder\Helpers\BuilderHelper::deleteBuilderCssOnPageDelete($page->permalink, $page->id);
            }
        }
    }
