...
class TagController extends Controller
{
    ...
    public function tag(Request $request)
    {
        try {
            ...
            return view('core::base.tenant.blog.tag.tag', compact('tags'));
        } catch (\Exception $e) {
    ...
    {
        return view('core::base.tenant.blog.tag.add_tag');
    }
    ...
    public function storeTag(TagRequest $request)
    {
        try {
            DB::beginTransaction();
            $this->blog_repository->addNewTag($request);
            DB::commit();
            ...
            DB::rollBack();
            toastNotification('error', translate('New Tag Creating Failed'));
            return redirect()->route('core.add.tag');
        }
    }
    ...
    public function editTag(Request $request, $id)
    {
        try {
            if ($request->id && is_numeric($request->id) && $request->lang) {
                ...
                return view('core::base.tenant.blog.tag.edit_tag', compact('tag', 'languages', 'lang'));
            } else {
