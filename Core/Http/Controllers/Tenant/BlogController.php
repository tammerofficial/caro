...
use Core\Repositories\Tenant\BlogRepository;
...
class BlogController extends Controller
{
  ...
  public function blog(Request $request)
  {
    try {
      ...
      return view('core::base.tenant.blog.blog', compact('blogs'));
    } catch (\Exception $e) {
  ...
  public function addBlog()
  {
    try {
      ...
      return view('core::base.tenant.blog.add_blog', compact('categories', 'tags'));
    } catch (\Exception $e) {
  ...
  public function editBlog(Request $request, $id)
  {
    try {
      if (isset($request->id) && is_numeric($request->id) && $request->lang) {
        ...
        return view('core::base.tenant.blog.edit_blog', compact('blog', 'categories', 'tags', 'languages', 'lang'));
      } else {
  ...
  public function blogPreview(Request $request)
  {
    try {
      ...
      $blog->tag = $blog_tag;
      return view('theme/tlcommerce::backend.blog_preview', compact('blog'));
    } catch (\Exception $e) {
      abort(404);
    }
  }
  ...
  public function categoryLoad(Request $request)
  {
    try {
      ...
      $categories = $this->blog_repository->getParentCategories();
      $view = view('core::base.tenant.blog.includes.category-select', compact('categories', 'blog_category'))->render();
  ...
  public function TagLoad(Request $request)
  {
    try {
      ...
      $tags = $this->blog_repository->getAllTags();
      $view = view('core::base.tenant.blog.includes.tag-select', compact('tags', 'blog_tag'))->render();
