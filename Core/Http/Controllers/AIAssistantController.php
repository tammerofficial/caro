...
class AIAssistantController extends Controller
{
    ...
    public function setAICredentials()
    {
        $api_config = DB::table('open_ai_settings')->first();

        $this->api_key = !empty($api_config) ? $api_config->api_key : '';
        $this->default_model = !empty($api_config) ? $api_config->default_model : '';
    ...
    public function updateAISetting(Request $request)
    {
        try {
            ...
            $enable_blog_ai_assistant = $request->has('enable_blog_ai_assistant') ? config('settings.general_status.active') : config('settings.general_status.in_active');

            setGeneralSetting('enable_blog_ai_assistant', $enable_blog_ai_assistant);

            DB::table('open_ai_settings')->updateOrInsert(
                ['id' => DB::table('open_ai_settings')->max('id')],
                [
    ...
    public function generateContent(Request $request)
    {
        ...
        try {
            ...
            if ($request['blog_details'] == "true") {
                ...
                $response = $this->generateAIContent('blog_details', $content_length, $request);
                if ($response['status']) {
            ...
            if ($request['short_details'] == "true") {
                ...
                $response = $this->generateAIContent('short_details', $short_content_length, $request);
                if ($response['status']) {
            ...
            if ($request['blog_title'] == "true") {
                ...
                $response = $this->generateAIContent('blog_title', $title_length, $request);
                if ($response['status']) {
     ...
     */
    public function generateAIContent($type, $max_token, $request)
    {
        $this->setAICredentials();
