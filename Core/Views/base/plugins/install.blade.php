                ...
                <form action="{{ route('core.plugins.install') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="form-row">
                        <div class="col-12 text-right">
                            <button type="submit" class="btn long" id="install-plugin-btn">
...
@section('custom_scripts')
    <script>
        $(document).ready(function() {
            'use strict'
            $('#install-plugin-form').submit(function(event) {
                event.preventDefault()
                $('#install-plugin-btn').prop('disabled', true)
