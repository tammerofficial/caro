    ...
    <div class="row">
        <div class="col-lg-6 mx-auto">
            <div class="form-element py-30 mb-30">
                <h4 class="font-20 mb-30">{{ translate('Install New Theme') }}</h4>
                @if ($errors->has('error'))
                    <p class="alert alert-danger">{{ $errors->first('error') }}</p>
                @endif
                <form action="{{ route('core.themes.install') }}" method="POST" enctype="multipart/form-data">
                    @csrf
