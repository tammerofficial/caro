...
Route::group(['middleware' => 'auth'], function () {
    ...
    Route::get('/media-page', [MediaController::class, 'mediaPage'])->name('core.media.page')->middleware(['can:Manage Media']);
    ...
    if (!isTenant()) {
        Route::middleware(['can:Manage Plugins'])->group(function () {
            ...
            Route::post('/plugin-inactive', [PluginsController::class, 'inactive'])->name('core.plugins.inactive')->middleware('demo');
    ...
    Route::middleware(['can:Manage Themes'])->group(function () {
        ...
        Route::get('/theme-install', [ThemesController::class, 'create'])->name('core.themes.create');
        Route::post('/theme-install-process', [ThemesController::class, 'install'])->name('core.themes.install');
    ...
    if (isTenant()) {
        ...
        Route::controller(TenantBlogController::class)->group(function () {
            ...
            Route::get('/add-blog', 'addBlog')->name('core.add.blog')->middleware(['can:Create Blog', 'check.subscription']);
    ...
    if (!isTenant()) {
        // Blog Category Routes
        Route::controller(CoreBlogCategoryController::class)->group(function () {
            Route::middleware(['can:Manage Category'])->group(function () {
                Route::get('/blog-category', 'blogCategory')->name('core.blog.category');
        ...
        //Tag Routes
        Route::controller(CoreTagController::class)->group(function () {
            Route::middleware(['can:Manage Tag'])->group(function () {
                Route::get('/tag', 'tag')->name('core.tag');
                Route::get('/add-tag', 'addTag')->name('core.add.tag');
                Route::post('/store-tag', 'storeTag')->name('core.store.tag');
        ...
        // Blog  Route
        Route::controller(CoreBlogController::class)->group(function () {
            Route::get('/blog-list', 'blog')->name('core.blog')->middleware(['can:Show Blog']);
        ...
        //Blog Comment Routes
        Route::controller(CoreCommentController::class)->group(function () {
            Route::middleware(['can:Manage Comment'])->group(function () {
                Route::get('/comments', 'comment')->name('core.blog.comment');
                Route::post('/comment/bulk/action',  'bulkAction')->name('core.blog.comment.bulk.action')->middleware('demo');
                Route::get('/comments/edit/{id}', 'editComment')->name('core.blog.comment.edit')->middleware('demo');
                ...
                Route::post('/comment-setting/update', 'updateCommentSetting')->name('core.blog.comment.setting.update')->middleware('demo');
                // both in frontend and core
                Route::post('/blog/comment/reply', [CoreCommentController::class, 'replyBlogComment'])->name('core.blog.comment.reply')->middleware('demo');
            });
        });

        // Page Routes
        Route::controller(CorePageController::class)->group(function () {
            Route::get('/page-list', 'page')->name('core.page')->middleware(['can:Show Page']);
            Route::get('/add-page', 'addPage')->name('core.page.add')->middleware(['can:Create Page', 'check.subscription']);
            Route::post('/store-page', 'storePage')->name('core.page.store')->middleware(['can:Create Page']);
            Route::get('/edit-page', 'editPage')->name('core.page.edit')->middleware(['can:Edit Page']);
    ...
    Route::group(['prefix' => 'site-map'], function () {
        if (isTenant()) {
            ...
            Route::post('generate', [SiteMapController::class, 'generateTenantSitemap'])->name('core.admin.site.map.generate')->middleware('demo');
            Route::get('download', [SiteMapController::class, 'downloadTenantSitemap'])->name('core.admin.site.map.download');
        }
        if (!isTenant()) {
            Route::get('', [SiteMapController::class, 'sitemap'])->name('core.admin.site.map');
    ...
    Route::group(['prefix' => 'backup'], function () {
        ...
        Route::get('generate-backup-file', [BackupController::class, 'fileBackupGenerate'])->name('core.backup.files.generate')->middleware('demo');
        Route::post('delete-backup-file', [BackupController::class, 'fileBackupDelete'])->name('core.backup.files.delete')->middleware('demo');
        Route::get('download-backup-file/{filename}', [BackupController::class, 'fileBackupDownload'])->name('core.backup.files.download')->middleware('demo');
        //Database backup
        Route::get('database', [BackupController::class, 'databaseBackup'])->name('core.backup.database.list');
        Route::get('generate-database-backup', [BackupController::class, 'databaseBackupGenerate'])->name('core.backup.database.generate')->middleware('demo');
        Route::get('download-backup-database/{filename}', [BackupController::class, 'databaseBackupDownload'])->name('core.backup.database.download')->middleware('demo');
        Route::post('delete-backup-database', [BackupController::class, 'databaseBackupDelete'])->name('core.backup.database.delete')->middleware('demo');
    });
    ...
    Route::group(['prefix' => 'system'], function () {
        ...
        Route::get('cancel-update', [UpdateController::class, 'cancelUpdate'])->name('app.system.update.cancel')->middleware('demo');
